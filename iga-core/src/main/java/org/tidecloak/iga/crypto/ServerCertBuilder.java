 ...
 *
 * <p>Ported verbatim from the {@code add-server-identity} branch
 * ({@code org.tidecloak.base.iga.serveridentity.ServerCertBuilder}). The only change is
 * the package declaration; the DER encoding is byte-identical so a cert assembled here
 * verifies against a CA built by the source.
 */
public class ServerCertBuilder {
     ...
     *
     * @param vvkPublicKeyBytes VVK public key bytes (gVVK)
