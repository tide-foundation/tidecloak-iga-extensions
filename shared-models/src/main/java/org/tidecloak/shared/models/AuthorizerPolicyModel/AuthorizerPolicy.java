...
public final class AuthorizerPolicy {
    ...
    private static byte[] b64ud(String s) { return Base64.getUrlDecoder().decode(s); }
    ...
    public byte[] bytesForSigning()         { return signingBytes; }         // bytes VRK must sign
    public String toCompactString()         { return compactNoSig; }         // "h.p"
    ...
    /** Optional: quick structural + BH/DLL parity checks; returns first error or null if OK. */
    public String validate() {
        ...
        // If dll included, verify bh == sha256(dll)
        if (payload.assemblyBase64 != null && !payload.assemblyBase64.isBlank()) {
            try {
                byte[] dllBytes = Base64.getDecoder().decode(payload.assemblyBase64);
                String hex = toHex(MessageDigest.getInstance("SHA-256").digest(dllBytes));
                if (!("sha256:" + hex).equalsIgnoreCase(payload.bh)) return "bh != sha256(dll)";
                if (payload.dllSize == null) payload.dllSize = (long)dllBytes.length;
            } catch (Exception ignored) { /* leave full verification to server vetter */ }
        }
        return null;
    }
