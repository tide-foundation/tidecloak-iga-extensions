...
| `multiAdmin` | **Unchanged** from today's `TideAttestor.record` (`TideAttestor.java:82-101`): enforce `IgaScopeResolver.requireApprover`, persist `approval = admin.getUsername()`. Dedup is enforced one layer up in `IgaAdminResource.authorize` (`IgaAdminResource.java:259-278`). |

In **both** modes the persisted shape is the same — `IgaAuthorizationEntity` with `approval = admin.getUsername()` — so the existing `combineFinal(... authorizations)` accumulator works unchanged.
